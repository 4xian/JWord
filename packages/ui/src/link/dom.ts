...
export function createLinkPanelDom(host: HTMLElement): JWordLinkPanelDom {
  ...
  configureQuickToolButton(openLinkButton, '打开链接', 'data-jword-link-open', 'openLink')
  configureQuickToolButton(editLinkButton, '编辑链接', 'data-jword-link-edit', 'paragraphStyle')
  configureQuickToolButton(removeLinkButton, '删除链接', 'data-jword-link-remove', 'trash')
  ...
  visibleTextInput.placeholder = '显示文本'
...
export function renderLinkPanel(
  ...
  const validationError = readLinkValidationError(state.dialog.draft, policy)
  ...
  dom.confirmButton.disabled = readLinkConfirmDisabled(state.dialog, policy)
  dom.confirmButton.textContent = state.dialog.mode === 'edit' ? '保存链接' : '插入链接'
}
