 ...
 * 约束: 断言来自页面 DOM 和公开 window.__jwordDemo hook，不包含正文、token 或 license private key。
 */
import { expect, test } from '@playwright/test'

test('Gate 7 vanilla opt-in devtools renders diagnostics summary without document text', async ({ page }) => {
  await page.goto('/?devtools=true')
  await page.waitForFunction(() => window.__jwordDemo !== undefined && window.__jwordDemo.devtools.isAttached())
  ...
  const snapshot = await page.evaluate(() => window.__jwordDemo?.devtools.refresh())
