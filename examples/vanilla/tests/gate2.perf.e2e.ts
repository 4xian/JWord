...
test('Gate 2 fixture exposes real browser scroll metrics and virtualization bounds', async ({ page, browserName }, testInfo) => {
  ...
  expect(metrics.initialMountedCanvasCount).toBeLessThanOrEqual(5)
  expect(metrics.peakMountedCanvasCount).toBeLessThanOrEqual(5)
  ...
  expect(metrics.maxCanvasSidePx).toBeLessThanOrEqual(4096)
