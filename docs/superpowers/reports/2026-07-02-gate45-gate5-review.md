...
| 编号 | 包 | 位置 | 问题 |
|------|-----|------|------|
| N-1 | native | `packages/native/src/index.ts:630,808,819,830,841` | metadata/document/checksums 解析错误统一误报为 `JWORD_NATIVE_MANIFEST_INVALID`，且缺少 `JWORD_NATIVE_METADATA_INVALID` 错误码 |
...
**N-1 错误码语义错误**（`src/index.ts:630、808、819、830、841`）
`parseJsonRecord`/`readString`/`readNumber`/`readStringArray` 把错误码硬编码为 `JWORD_NATIVE_MANIFEST_INVALID`，被 `readMetadata`、`readDocument`、`readChecksums` 复用。`metadata.json` 或 `checksums.json` 损坏时报"manifest 无效"，诊断误导调用方。types.ts 中也不存在 `JWORD_NATIVE_METADATA_INVALID`。
**修复建议**：让 `parseJsonRecord` 等函数接收 error code 参数；补充 `JWORD_NATIVE_METADATA_INVALID`、`JWORD_NATIVE_DOCUMENT_INVALID`、`JWORD_NATIVE_CHECKSUMS_INVALID` 的针对性使用。
...
- **manifest.json 自身不受 checksum 保护**（`src/index.ts:441` 附近 `createChecksums`）：只覆盖 document.json、metadata.json 和资源文件；篡改 manifest 的 `featureFlags`/`resources` 列表无法被检出。建议把 manifest.json 纳入 checksums，或增加包级摘要。
...
- **批注文本读取逻辑有误**（`src/package.ts:671-678`）：`readXmlChildren(text).length === 0 ? readElementText(text) : ''` —— `w:t` 一旦含子元素其文本被清空为 `''`，该 children 判断多余且有害，批注正文可能丢失。
...
- **roundtrip snapshot 覆盖缺口**（`src/roundtrip.ts:130-220`）：run.link（超链接目标）、run.field、段落 tabs、bookmark、comment 数据均不在 snapshot 中，这些能力的 roundtrip 回归是盲区（hyperlink 属 T1，其余多为 T2）。
...
- **P2（商业发布阻塞项）签名强度不足**（`src/index.ts:227-236`）：`createStableLicenseHash` 用 32 位 FNV-1a，输出 8 个十六进制字符，碰撞和伪造成本极低；且 verifier material 为 `jword-local-verifier:${issuer}`（`src/index.ts:222-224`），知道 issuer 即可伪造合法签名。注释已声明"非密码学签名、仅离线测试契约"，开发阶段可接受，但 **Gate 6/商业发布前必须替换为 HMAC-SHA256 或非对称签名**。`tests/architecture/gate5-commercial-readiness.test.ts` 建议增加"发布前签名算法升级"检查项。
...
- **P3 外部证据全部 pending**：`compatibility-matrix.json` 中 Word/LibreOffice/OpenXML validator 结果全为 pending，`openxml-validation-results.json` 为空集。当前 WPS 手动证据合格，但 "Transitional 标准合规" 缺 OpenXML validator 证据支撑（这与 D 部分发现的 `w:shd` 缺 `w:val` 问题相关——validator 一旦接入很可能报错）。
...
| roundtrip diff T1 覆盖 | 覆盖文本/样式/列表/表格/图片/节页面设置；**hyperlink、field、tabs、bookmark、comment 不在 snapshot**（P3，hyperlink 属 T1 盲区） |
...
1. **立即修复（P1）**：D-1（toggle 语义）、P-1（PDF 文本样式）、P-2（Latin-1 误判）、D-2（多 section 至少补 warning）、N-1（错误码）、N-2（迁移链）。
2. **本 Gate 内（P2）**：批注文本读取 bug（package.ts:671-678）与 commentRangeEnd 丢弃、DOCX worker progress 接线与 cancel 竞态、XML 数字字符引用/CDATA/namespace 继承、`w:shd`/`w:u` schema 合规（配合接入 OpenXML validator 证据）、负页边距、normalizePartPath 防御、PDF subset 与 fallback chain、native manifest checksum 与 zip 防护、示例 Worker 路径闭环（X-1）。
3. **商业发布前（阻塞 GA）**：license 签名算法替换为密码学签名。
4. **持续改进（P3）**：roundtrip 补 hyperlink、媒体字节内存优化、重复工具函数抽取、PDF 颜色/页面尺寸边界。
