...
2. `docs/superpowers/reports/2026-07-03-remediation-execution-supplement.md`
   —— 执行补充：预置决策 D1-D9、时点漂移处理规范（§2）、大任务拆解（§3）、人工检查点（§4）。**优先级规则：凡其任务映射表收录的任务，以补充文档为准；两文档冲突时以补充文档为准。**
...
分批节奏：每完成一批（3-5 项）暂停，输出阶段报告，等人工确认后继续。第一批 = 下一步行动 1-6 项。15 项完成后继续 Phase 1 剩余 → Phase 2 → Phase 3…，大任务（表格跨页、PDF 样式、字体度量、性能专项、文件拆分、Plugin API 等）一律按补充文档 §3 对应章节的子步骤执行。

## 人工检查点（补充文档 §4，到点必须暂停等确认）

1. 第 0 步基线报告后。
2. 每批阶段报告后。
3. Plugin API 的 M1 设计文档完成后与 M5 内部消费者验证后。
4. D6 浏览器矩阵写入对外文档前。
5. 字体度量 §3.6 阶段二（切换真实测量、大面积刷新视觉基线）动工前。
6. 任何真实 publish 动作（永久禁止自动执行）。
...
6. 回写文档：在修复计划勾选 checkbox，行尾追加「完成 YYYY-MM-DD：修复摘要；验证命令与结果」；按补充文档执行的任务同时记录执行到的子步骤编号。

每批结束全量回归：`pnpm lint && pnpm typecheck && pnpm test`；涉及布局/渲染加跑 `pnpm test:visual`（基线刷新必须逐文件列明，不允许一句带过）。
