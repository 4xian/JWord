...
| JWR-P0-005 | 默认集成 | 动态复验/第二批完成 | `createJWord({ host })` 已成为 Quickstart 和默认 vanilla 唯一集成入口；内部固定上中下结构并统一 destroy。默认 demo 不再接线 `__jwordDemo`，复杂 Gate 能力只存在于独立 `/test-fixture.html`。 | `packages/ui/src/editor-shell.ts`；`docs/sdk/quickstart.md`；`examples/vanilla/src/main.ts`；`examples/vanilla/tests/editor-shell.e2e.ts` | 单 Host 源码集成、demo、Quickstart、销毁与 Chromium smoke 已通过；仍需在发布批次完成 tarball 空项目的编辑、重开和销毁消费验证。 | In Progress（第二批范围 Done；tarball 消费待 JWR-P0-008/P1-111） |
| JWR-P0-006 | 基础门禁 | 运行复现/typecheck 与 lint 已复验 | `pnpm typecheck` 和 `pnpm lint` 已恢复；focused core/toolbar 文件预算仍为已知红灯。第二批默认 demo 已删除生产 `__jwordDemo`，测试能力迁入 `__jwordTestFixture`。 | `examples/vanilla/src/main.ts`；`examples/vanilla/src/vite-env.d.ts`；`examples/vanilla/tests/fixtures` | 2026-07-10 根 `pnpm typecheck` 与 `pnpm lint` 均 exit 0；JWR-P0-006 仍需后续 focused file-budget 退出 0。 | Open（typecheck/lint Done；file-budget 未完成） |
| JWR-P0-007 | 发布产物 | 运行复现 | release dry-run 假绿；当前 core dist 缺 Node ESM 后缀，真实 import 失败。 | `tools/release/gate7-release-dry-run.mjs:70-146`；`tools/release/normalize-dist-relative-imports.mjs:17-19` | fresh build 后 normalization check、全部入口 dynamic import 和 dry-run 同一 SHA 通过。 | Open |
| JWR-P0-008 | 第三方消费 | 运行复现 | no-alias smoke 在 pnpm 安装阶段请求未发布 registry 包，未进入 type/build/browser 阶段。 | `tools/release/check-gate7-third-party-smoke.mjs:142-192`；`package.json:6` | 锁定 pnpm 9.14.2 的 tarball 传递依赖可解析，完整 consumer smoke 通过。 | Open |
...
| JWR-P2-206 | 大文件 | 行数/门禁 | toolbar DOM/controller、UI lifecycle/types、layout query 仍承担过多职责。 | `packages/ui/src/toolbar/dom.ts` 等 | 分批按 state/event/view/普通面板拆分，行为与 API 不变。 | Open |
...
| JWR-P2-211 | 国际化 | 能力缺口 | 当前重点是中英文，完整 RTL 和更广语言/字体/输入矩阵未完成。 | `docs/current-implementation/backlog.md:237-247` | 语言、RTL、字体和输入法支持矩阵通过。 | Open |
