...
2026-07-12 补充整改：默认工具栏此前会渲染批注、链接、页眉/页脚/页码、查找替换、目录和修订入口，但裸 `createJWord({ host })` 没有创建对应 controller。现已改为由最终工具栏配置驱动内建能力装配：工具可见时自动使用内部 editor/panel Host，工具被隐藏时不额外装配；显式 UI 配置和高级 slots 继续优先。查找替换面板改为锚定工具栏按钮下方，修订面板在 EditorShell 中间编辑区域内居中显示，两者都不再作为普通流内容落到编辑区底部；临时下拉和弹框在打开新入口或点击外部时互斥收起，内部点击不关闭，select 选择值后关闭。目录与批注仍是持续工作区，不套用临时浮层关闭语义。
