...
> 文档状态：`Implementation Ready / final evidence pending`（B4 checkpoint 1 complete）
>
> 调查日期：2026-07-21
>
> 实施顺序唯一来源：[09-remediation-roadmap.md](09-remediation-roadmap.md)。本文件规划并记录统一路线 Phase 3；不执行真实 publish，也不进入 OEM Phase 3。
>
> 当前计划复审：Standards `PASS / 0 finding`；Spec `PASS / 0 finding`（原始计划第26轮冻结稿 SHA-256 `f9b4a5e0b6bc3793495d1509f97fad334a3370e80014ff9b5e3bc6db2d72a6bc`；B4检查点1冻结稿 SHA-256 `a70c6e3f2e90ec6a83c6531263cbc5b6c2e8e460cf2f5842bc9dfd087f3cbfca`）。B0、B1、B2、B3实现及B4检查点1均已对同一冻结快照取得Standards与Spec `PASS / 0 finding`，当前保持`Implementation Ready / final evidence pending`，不得进入B4检查点2或B5。
...
| `tools/release/build-phase3-artifacts.mjs` | B2 | source report、clean检查、canonical/repro build、staging pack、inventory/checksum |
...
| `tests/architecture/phase3-artifact-build.test.ts` | B2/B4 | B2锁定builder fail-closed与inventory schema；B4锁定reproducibility固定payload及`evidence-manifest.json` |
...
- 新增B4十个文件：7个tool、`fixtures/release/rollback-state.json`、`fixtures/release/release-policy.json`和`tests/architecture/phase3-release-gates.test.ts`；修改`.github/workflows/ci.yml`、`package.json`、`tools/release/compare-phase3-artifacts.mjs`、`tests/architecture/phase3-artifact-build.test.ts`、`tools/visual/run-visual.mjs`、`tests/gate2-fixture.test.ts`和本文件，见第7节。
...
Focused阶段只验证B4脚本/CI contract的synthetic fixture，不要求尚未生成的JWord artifact manifest；`check-phase3-release-gates.mjs`真实candidate模式只在下述最终expanded pipeline运行。
