...
- canonical packed manifest 把内部 `workspace:*` dependency/peer 精确改写为同一 artifact set 的 `0.0.0`。因为这些版本不在真实 registry，B3 的每条 journey 必须从 contract 计算完整 first-party 传递闭包，但空项目根 manifest 只以精确版本显式列出`requestedPackages`与精确external peers；只读 loopback scoped registry从hash绑定的真实packed manifest生成metadata，使`firstPartyClosure`只能经依赖关系解析到同一 run-a tarball bytes。禁止安装全量无关包、override、resolution、alias 或 workspace link。该证据证明同批 tarball 的名称/版本传递解析和消费，不证明真实 registry access、权限、2FA、signed provenance 或 publish readiness；这些仍是外部发布 gate。
...
1. `tools/release/package-artifact-contract.json` 是 package 分类、tarball allowlist、export运行环境、journey target映射和Phase 3 size budget的唯一机器可读真源；Gate 5/6/7 不再各自维护 package 数组。每个export entry恰好记录`subpath`、`target`和非空`environments[]`，环境枚举固定为`node`、`browser`、`dedicated-worker`、`types`、`image-node`：11个npm-delivery package的`.`恰好标记`node/browser/types`，其中License根入口额外标记`dedicated-worker`；UI `./styles.css`只标记`browser`，native/docx/pdf `./worker`标记`browser/dedicated-worker/types`，Collab `./experimental`标记`node/browser/types`，`collab-server`的`.`只标记`image-node/types`。每个journey保存精确且非空的`targets[]`，target恰好为`{ package, subpath, environment, runtime }`，引用一个已声明export/environment，且runtime只能是本journey声明值之一；映射固定为`node -> node`、`browser -> vite-browser`、`dedicated-worker -> dedicated-worker`、`types -> types`、`image-node -> image-node`，不得跨类复用标签。B3只能从这些映射生成tuple和逐export证据，禁止在Node动态import CSS或worker-only entry，也禁止把未分类entry静默跳过。
   ...
   contract中的budget集合恰好为：一个`core-entry-js`、一个`native-registry`以及`vanilla-first-screen/{npm|pnpm}/{chromium|firefox|webkit}`六项；`core-entry-js.limitBytes`固定为`650000`，六个Vanilla budget的`limitBytes`固定为`900000`，`native-registry.limitBytes`固定等于implementation base中`packages/native/fixtures/registry.json`的实际bytes且同时冻结该文件SHA-256，Phase 3不允许其增长或换内容。Vanilla新limit以真实完整首屏`859055` bytes为基线，保留`40945` bytes、约`4.77%`的有限余量；后续真实值不得自动抬高该冻结值。`core-entry-js.source`固定为`tarball:@4xian/jword-core/dist/index.js`，对应tarball内部相对path是`dist/index.js`；`native-registry.source`固定为`tarball:@4xian/jword-native/fixtures/registry.json`，对应tarball内部相对path是`fixtures/registry.json`；两者的`sourceSha256`取该单文件原始bytes。install ID固定为`<journey>--<packageManager>`，source/bundle ID固定为`<journey>--<packageManager>--<runtime>--<browser>`；六个Vanilla `source`分别固定为`consumer:bundles/vanilla-editorshell-css--<npm|pnpm>--vite-browser--<chromium|firefox|webkit>/first-screen`且不得重复。每项选择对应bundle目录`index.html`直接引用的首屏JS/CSS regular file集合，包括Vite写入HTML的`modulepreload` JS。实际HTML和全部bundle文件必须在consumer handoff内，`bytes`为该集合总和，`sourceSha256`为按POSIX path排序的`{ path, sha256, bytes }`数组按第7项canonical serialization所得SHA-256。

   2026-07-24真实run-a/consumer evidence按上述口径重算得到Core `612816 / 650000`、native registry `1093 / 1093`，六个Vanilla tuple均为`859055 / 700000`；consumer HTML直接引用入口JS、CSS和Vite生成的`modulepreload` chunk，不能通过删除`modulepreload`或缩窄统计集合隐藏立即加载bytes。既有`pnpm size` fresh build仍通过，但其`tools/size/check-size.mjs`只统计示例HTML中的module script与stylesheet，不统计`modulepreload`，且示例入口形状不同，因此其`69634`不能替代本阶段同一tarball consumer evidence。用户于2026-07-25明确选择方案2，六项Vanilla limit据此统一冻结为`900000`；这是一次经批准的Phase 3 rebaseline，不授权后续按新观测值自动抬高预算。
...
15. SBOM固定为SPDX 2.3 JSON；依赖图只读取repo外`customer-production`和`server-image`两套assembly各自的`pnpm list --prod --depth Infinity --json`结构化输出、独立assembly lockfile和packed manifests。assembly lockfile是该assembly依赖图、精确版本及完整`optionalDependencies`集合的唯一lockfile来源；repo `pnpm-lock.yaml`不参与依赖图或版本选择。assembly生成端只把当前OS/arch实际物化且可解析物理路径的依赖写入`dependencies[]`；pnpm list可能列出assembly lockfile中声明但当前平台未物化的optional package，此类节点保留在raw list，并仅在已物化父包真实`package.json.optionalDependencies`声明该名称时写入独立`unmaterializedOptionalDependencies[]`，不得伪造成已安装依赖或按结果集合自行过滤。每个非根级raw dependency无论物化或未物化，都必须把name/version绑定到pnpm virtual-store路径身份，即路径结构必须连续为匹配的`.pnpm/<encoded-name>@<version>.../node_modules/<name>`，virtual-store entry与`node_modules`之间不得插入额外目录；长package base identity必须按pnpm v9的93字符前缀加lowercase MD5 base32规则接受，含peer-context时预检只验证该固定长目录形状，随后必须由assembly lock中的完整snapshot identity重算并精确绑定hash，禁止要求哈希目录仍以完整base identity开头或只信前缀形状。同一name/version存在多个peer-context snapshot时，必须按pnpm v9 virtual-store目录编码从raw物理路径恰好命中一个具体snapshot，禁止只按依赖集合择一，零命中、重复key/identity或多个snapshot编码为同一目录名都必须fail closed。required `@4xian/*` first-party直接根可使用项目根级解析路径，required external直接根仍必须绑定自身`.pnpm`身份。raw直接依赖name/version集合必须与contract `requiredDependencies`精确相等，final job还必须证明raw闭包中的每个节点恰好属于`dependencies[]`或显式optional记录之一。assembly lockfile必须逐节点覆盖raw闭包，并从raw路径唯一绑定的同一snapshot复核普通与optional完整子集合及未物化子snapshot的`optional: true`，禁止从另一peer-context借用optional标记；父snapshot中的每条普通或optional edge必须保留完整resolution peer-context，并按pnpm v9目录编码与raw child的virtual-store物理路径逐边一致，同一child name/version同时出现在`dependencies`和`optionalDependencies`时必须fail closed。同一物理父节点被raw图从独立分支重复展开时，只允许完整child map的name/version/path和optional分类完全一致，任一不同子集合或冲突edge必须fail closed，禁止合并成并集或用后一次展开覆盖前一条证据；仅当当前递归栈内回到同一物理父节点时，允许空child map作为pnpm list循环终止标记，非空循环回边仍须fail closed。verifier还须复核父snapshot的精确`optionalDependencies`边及`packages:`区段的`os`/`cpu`约束确实排除run-a冻结环境。删除任一已物化传递依赖、伪造optional、缩小完整optional集合或让两组重叠都必须失败，同时要求contract直接依赖全部物化。当前checkout中raw SHA已由run-a identity绑定的repo `pnpm-lock.yaml`只对实际被分类为未物化optional的记录做独立复核：同一父name/version snapshot必须以精确子name/version声明该optional边，且同一子name/version的`packages:`约束必须排除run-a冻结环境；不得要求其他物化父子节点存在于repo lock，不得从repo lock枚举整个assembly optional集合，也不得因repo workspace与assembly的peer-context或解析版本不同拒绝合法assembly。若上一台runner的绝对路径在final runner仍存在，则继续执行realpath/symlink校验，若跨job已不存在，则只允许绝对、规范化且位于仓库外的记录路径，不得假设上一台runner的assembly目录仍存在。同一SPDX document必须建立两个不同的assembly root package及其关系，客户root覆盖11个npm-delivery tarball闭包，server root覆盖`collab-server`及contract计算的image-internal first-party闭包；不得合并依赖树或让server依赖伪装成客户依赖。first-party legal license未批准时写`NOASSERTION`。
...
16. `pnpm audit --prod --audit-level high --json`是依赖外部registry的自动gate，必须分别在repo外`customer-production`和`server-image` assembly目录执行。pnpm `9.14.2`仍会在JSON中保留info/low/moderate计数，并可能在high/critical均为0时因这些advisory返回exit 1；脚本必须按原始bytes保存完整JSON stdout并解析有效payload，同时保留命令stderr参与fail-closed分类。任一high/critical计数非零为`failed`；有效JSON、空stderr、exit 0且high/critical均为0时通过；有效JSON、空stderr、exit 1、high/critical均为0且至少存在一项info/low/moderate advisory时也继续按本阶段high/critical门槛通过。无有效JSON、顶层`error`/`errors`字段、任何stderr、非0/1状态，或非零退出且没有任何可归因advisory时为`blocked`，不得把registry/网络/工具异常伪装成绿灯。任一目录缺少原始报告时B4 closure blocked，不能用另一套assembly的结果替代。两套audit/list原文、摘要、SHA-256和各自assembly manifest/lockfile都进入固定evidence。
...
| `tools/release/package-artifact-contract.json` | B0 | 12 包机器可读分类、逐export环境标签、allowlist、dependency policy和固定size budget；Vanilla经批准rebaseline为`900000`后继续fail closed，禁止按后续观测值自动抬高limit |
...
| `tools/release/phase3-artifact-utils.mjs` | B2/B4 | B2提供canonical JSON、hash、inventory读取/校验共享函数；B4由唯一`TEST_COMMANDS`常量同时冻结builder执行与final verifier校验的四条命令，并从hash绑定tarball读取真实packed manifest供consumer metadata使用；E2E/benchmark定义保持Phase 3专属script，id、顺序、direct Vitest和visual不变 |
...
| `tools/release/phase3-assembly-dependencies.mjs` | B4 | 物化依赖、未物化optional记录及跨job路径的共享校验 |
...
- 写入本文件第4/6节已经冻结的JSON contract和对应说明；contract必须按精确target映射计算每条journey的tuple、逐export期望集合和最小first-party依赖闭包，并把native唯一fixture例外记录为精确路径而非目录通配；同时写入8个固定size budget及第6节冻结的source，native baseline bytes/hash直接读取并绑定implementation base中的精确文件。Vanilla只允许本次基于真实完整首屏证据、经用户批准的`900000` rebaseline，后续不得按运行结果自动抬高limit；不改任何source manifest。
...
- runner的production模式要求`--artifact-manifest`与`--binding`同时显式提供且先按第6节第14项完成校验；npm/pnpm各自使用完全独立空项目、userconfig、cache/store和第6节第9项只读loopback registry。每条journey的package.json只把`requestedPackages`以精确版本列为direct dependencies并列出精确external peers；`firstPartyClosure`仍由loopback registry提供，但只能经hash绑定tarball manifest生成的metadata依赖关系解析。first-party scope只从本次registry解析到run-a tarball，外部依赖使用官方registry。不安装全量无关JWord包，无overrides/resolutions/workspace/file/alias；生成后把原始package manifest、对应lockfile、包管理器结构化dependency tree、无凭据`.npmrc`、registry evidence、canonical transcript及其全部metadata/tarball raw response bytes按第6节第20项的固定文件名复制到consumer handoff`raw/installs/<installId>/`，计算hash并登记到`install-evidence.json`。其中`requestedPackages`只写journey直接请求的first-party package，`firstPartyClosure`只写其余contract派生传递依赖，根manifest的first-party集合必须精确等于`requestedPackages`，registry与resolved dependency tree的first-party集合必须精确等于两者并集；两个字符串数组分别ASCII排序、去重且不相交。npm/pnpm lockfile分别按第6节第9项的不同可执行字段校验，禁止要求pnpm v9 lock保存动态loopback origin。
...
```bash
pnpm exec vitest run tests/architecture/phase3-release-gates.test.ts tests/architecture/phase3-release-gates-assembly-install.test.ts tests/architecture/phase3-artifact-build.test.ts
...
| production dependency audit | 自动/外部registry | 两个assembly目录分别执行`pnpm audit --prod --audit-level high --json` | 有效JSON、空stderr、customer/server各自high/critical为0；exit 0通过，或exit 1且至少一项info/low/moderate advisory时通过；两套原始report/hash留证，payload error/外部错误与漏洞失败分开分类 | 必须；任一服务不可用则blocked |
...
#### P3-B4 remote perf稳定阻断与诊断边界（2026-07-24）

- WebKit预算修复的clean SHA `34cc10f0e358fd63a3b794e62be7ad34191c55f5`触发run `30068439502`。attempt 1与用户授权的failed-job attempt 2均通过`source-gates`，`artifact-build`也均越过direct Vitest、Chromium/Firefox和WebKit；WebKit结果稳定为65 passed / 3 skipped / 2 flaky，原六项30秒timeout不再出现。两项Gate 2仅首轮`expect.poll` 5秒超时，重试通过，不再阻断命令。
- attempt 1的Gate 3 `largeDocumentInsertP95Ms`为89.1/82.7/82.1ms，Gate 4 `findUiInteractionMs`为1410/1443.2/1434.5ms；attempt 2分别为89.7/89/85.2ms与1414.5/1411/1510.5ms。attempt 2还显示Gate 3 `alphaLoadMs`为89/100.1/92.9ms、Gate 4 `overlayCompositeScrollMs`为810.3/778/828.1ms，分别超过80ms与700ms。两轮相同量级证实这是稳定blocker，不再以rerun作为修复。
- 本机未节流的同一Gate 3/Gate 4 public seam连续五轮为34.1至35.3ms与667.4至690.3ms；Chromium 2倍CPU节流后可稳定复现Gate 3约73.6至78.1ms、Gate 4约1333至1368.1ms。当前排序假设为：共享runner绝对时延不可移植；单字符事务重复执行layout/render/status统计；查找为2400个命中重复创建稳定range并读取overlay布局；长链前置任务污染runner。必须用单变量计时探针区分，不得从本机绿灯或一次远端结果反推结论。
- Gate 4单变量探针已把生产重复工作定位到`packages/ui/src/find-replace/controller.ts::renderFindReplaceOverlay()`：同一公开交互在1倍/2倍CPU下分别约442.4/870.4ms，其中2倍CPU下`createTextAnchor`执行4852次、合计约322ms，`captureRangeSnapshot`执行2400次；overlay窗口实际只读取26次`getSelectionRects`，但经facade重复解析anchor和transient layout，合计约450.3ms。该函数已经持有一次`editor.getLayout()`和`locateRangeSnapshot()`返回的`TextPosition`，因此本批唯一生产修改是直接调用Core公开纯函数`getSelectionRects(layout, range)`；保留`focusActiveMatch()`的真实anchor/selection、2400个稳定range snapshot、24项overlay窗口、DOM结构、active match与滚动行为，不改Core facade、阈值、测试数据或Phase 3命令。先后复跑`pnpm exec playwright test examples/vanilla/tests/phase3-find-diagnostic.perf.e2e.ts --project=perf-chromium --workers=1`、`pnpm exec playwright test examples/vanilla/tests/gate4.perf.e2e.ts --project=perf-chromium --workers=1`和`pnpm exec vitest run packages/ui/test/find-replace-controller.test.ts packages/ui/test/create-ui-find-replace.test.ts`；Gate 3候选仍须单独量化，不能随本项一起实施。
- Gate 3事件探针确认单字符输入按`transaction`、`selectionChange`顺序连续刷新状态栏，且两次事件读取到同一个projection对象；2倍CPU profile中两次`createStatusBarTextStats`分别约7.9/6.7ms。第二个单变量生产修改限定为`packages/ui/src/status-bar/controller.ts`：按Core当前不可变projection对象身份缓存`createStatusBarDocumentStats()`结果，同一projection上的selection、scroll、resize、theme、locale与view refresh只复用文档统计，新projection仍立即重算；选区文本、页码、缩放、全屏、演示、主题与locale继续逐次读取，不改状态栏公开DOM或Core事件语义。先复跑临时事件探针，再依次运行`pnpm exec playwright test examples/vanilla/tests/gate3.perf.e2e.ts --project=perf-chromium --workers=1`和`pnpm exec vitest run packages/ui/test/create-ui-status-bar.test.ts packages/ui/test/status-bar-state.test.ts`；`syncPageWrappers()`几何读取和字体测量继续保持未实施，只有本项实测不足时才分别诊断。
- 状态栏缓存后的正式Gate 3首轮仍以`largeDocumentInsertP95Ms=57.5ms`超过50ms，说明该收益可保留但尚未关闭门禁。当前53页fixture的`packages/core/src/editor/rendering.ts::syncPageWrappers()`在每页`updatePageElement()`/`insertBefore()`写入后立即调用`alignPageWrapperToDevicePixels()`，每次重新读取同一`canvasContainer.clientWidth`与`getBoundingClientRect()`；2倍CPU profile命中该函数约6.1ms。第三个单变量只把对同一次渲染恒定的容器宽度/左边界移到全部wrapper同步完成后读取一次，再沿用原`twipsToCssPx`、居中和device-pixel round公式逐页写margin；不跨渲染缓存、不改变page wrapper/canvas集合、DOM顺序、48px页间距、分页layout或公开API。复跑同一Gate 3 perf、`pnpm exec vitest run packages/core/test/editor/runtime.test.ts`和Core package test；字体测量仍保持未实施。
- wrapper几何读取优化后Gate 3单轮为33.9ms，但三次重复为53.3/56.7/34.2ms，仍未稳定关闭50ms；新profile的最大可归因同步项变为一次全文`createStatusBarTextStats()`约9.3ms。真实projection结构共享探针确认单字符事务前后共有1200个顶层块，其中1199个保留相同冻结对象、只有被编辑首块换引用。第四个单变量限定为`packages/ui/src/status-bar/state.ts`：按只读`Block`对象身份用`WeakMap`缓存每块的词数、字符数和递归段落数，文档级只做数值聚合；新块和新table立即重算，未变块复用，Latin token仍先按完整paragraph跨run拼接后统计，block/paragraph边界仍等价于原换行分隔，不延迟状态栏、不改变公开类型或统计口径。`packages/ui/test/status-bar-state.test.ts`只增加一个共享旧块并替换新块的最小回归，再复跑状态栏focused test和同一Gate 3重复三轮；若稳定通过则停止，不进入字体测量。
- 四项最小生产优化的当前结果必须分开判定：Gate 4正式seam首轮为467.5ms，三次重复为460.8/465/496.8ms，清理诊断文件后为433.6ms，最后一次`syncPageWrappers()`简单微调后为433.2ms，稳定低于1200ms；对应overlay composite为333.1、354.3/346.6/340.3、290.4和291.9ms，稳定低于700ms。Gate 3同步dispatch已降至1倍约8.2ms、2倍约11.3ms，单字符事务不再触发新的原生canvas文本测量，但正式20样本p95三轮仍为56.5/52.4/54.1ms而失败。无操作20样本双rAF基线本机1倍p95已为47.3ms、2倍为59.2ms，证明剩余超线由绝对帧时延模型主导，不能继续侵入字体/layout生产路径，也不能放宽50ms阈值、删掉perf gate或改等待数值绕过。
- 当前远端执行能力也已核验：GitHub API显示仓库Actions专用runner数量为0，run `30068439502`的`artifact-build`实际label仅为共享`ubuntu-latest`，因此专用runner路径当前不可用。用户已批准跨runner可移植指标重设计，原runner/方案选择阻断已解除；这不等于远端Gate 3或B4 checkpoint 2关闭，仍须由包含当前完整差异的新clean SHA在同一共享runner six-handoff pipeline证明。当前最小生产差异已完成focused Core/UI、完整Core 73 files / 371 tests、完整UI 42 files / 185 tests、Phase 3 architecture 7/7、typecheck、lint、public type tests和两类diff验证，临时诊断文件已删除；B5、JWL1及后续Phase继续hard stop。
- 用户已批准第二条进入路径，并允许增加指标采样时间。Gate 3仍使用真实hidden textarea输入、真实Core transaction/layout/render和状态栏public seam，但两项已被远端证实runner-sensitive的绝对帧指标改为配对基线增量：大文档输入使用30组交错采样，每组先测量与`runAndMeasure()`相同的双`requestAnimationFrame`空操作基线，再执行单字符输入，分别报告原始p95、帧基线p95和逐样本`max(0, raw - baseline)`后的增量p95；Alpha load也使用30组独立配对，每组从已settled的大文档状态先测双帧基线，再通过公开“加载 Alpha 样例”按钮计量真实load，组间通过公开“恢复 Gate 2 夹具”按钮重建大文档。恢复完成以当前`packages/*`生产投影的原首段文本和1200个顶层块为准：实际运行态在Alpha之后虽恢复完整文本投影，但page count为1且首页选择按钮仍可用，不能继续把旧53页/按钮禁用假设当作完成predicate。测试级预算仅从60秒增至180秒以容纳30组恢复采样，两项性能门禁仍分别固定为增量p95不超过50/80ms，不提高阈值、不减少真实工作、不用固定常量抵扣。精确修改范围只含`examples/vanilla/tests/gate3.perf.e2e.ts`和本计划；Phase 3命令、root E2E、全局Playwright配置、workflow与`packages/*`生产实现保持不变。
- 旧指标在当前工作区以`largeDocumentInsertP95Ms=56.6ms`稳定复现同一红灯；旧Alpha单次配对被独立review判定不能证明跨runner可移植性。Alpha多组实现的首轮反馈环先因旧53页/按钮禁用恢复predicate超时，截图和真实投影确认已恢复1200块Gate 2文本后改按生产投影判定；随后focused三轮均通过且单轮约19秒。三轮大文档raw/baseline/incremental p95分别为`55.9/35.2/22.7ms`、`55.7/34.2/22.8ms`和`56.2/34.1/23.7ms`；Alpha 30组raw/baseline/incremental p95分别为`31.3/42.4/0ms`、`32.9/41.6/0ms`和`32.2/43/0ms`。两项增量均在原50/80ms阈值内，当前无数据支持放宽阈值。最终完整`pnpm test:e2e:phase3`为Chromium/Firefox 136 passed / 4 skipped、WebKit 67 passed / 3 skipped、perf-chromium 4 passed；其中Gate 3大文档raw/baseline/incremental p95为`55.7/33.4/22.2ms`，Alpha 30组为`32.2/44.1/0ms`，Gate 4为`450.7/301.6ms`。`pnpm typecheck`、`pnpm lint`、Phase 3 architecture 7/7和两类diff check均exit 0；单reviewer combined复核的Standards、Spec均为`PASS / 0 finding`，确认上一轮Alpha单次配对blocker已关闭，逐样本截零后的0ms p95表示至少29/30组raw未超过同组帧基线，并非单样本假绿。既有architecture contract继续锁定Phase 3三段命令、root E2E不变和全局配置无worker覆盖，本次指标契约由同一public perf文件的输出与断言自包含，不新增源码字符串耦合测试。
- 诊断阶段只允许读取和临时探针；所有临时`[DEBUG-phase3-*]`、CDP throttling、profile与throwaway文件必须在形成正式差异前删除。若证据指向生产重复工作，须先把精确`packages/*`文件、同一perf回归seam和最少package测试回写本节，再实施严格等价优化；若证据指向共享runner模型，须只修订Phase 3专属CI执行方式和对应architecture contract，不改根`test:e2e`、全局Playwright配置、既有阈值或后续Phase。两条路径均不得触及JWL1、DOCX、PDF、Collaboration、License trust/token或B5八文档。
- clean SHA `5a2a0d299917dfbd63d1446093d1d72911b37b54`触发run `30083564714`，`source-gates`通过，`artifact-build`的direct Vitest、Chromium/Firefox、WebKit与Gate 3新指标均通过；唯一明确blocker是Gate 4 `overlayCompositeScrollMs`三次为`754.4/762.6/737.3ms`，超过`700ms`。同三次`overlayScrollMs`为`626.2/528.5/556.5ms`且其余Gate 4阈值均通过；下游四个job仍跳过，未生成最终ID。
- 现有composite seam的两次滚动各等待一次双`requestAnimationFrame`，绝对指标因此把四帧共享runner调度与真实overlay工作混在一起。本机focused连续10次的旧值为`294.7至337.1ms`；临时4倍CDP CPU节流可使同一public seam的旧值达`1223.3ms`并稳定转红，证明该绝对墙钟指标不能跨runner直接比较。重读`packages/*`真实滚动路径后，当前没有新证据支持继续侵入Core/UI生产实现。
- 用户已允许增加指标时间，因此Gate 4 composite改为30组同轮配对采样：每组先测与真实两次滚动相同的四帧空操作基线，再保留原`scrollTop=0`、滚到底部、两次`scroll`事件与四帧等待的真实public flow；报告raw p95、frame baseline p95和逐组`max(0, raw - baseline)`后的incremental p95。性能阈值仍是incremental p95不超过`700ms`，未提高阈值；只把测试预算从60秒增至120秒。正式修改范围仅为`examples/vanilla/tests/gate4.perf.e2e.ts`与本计划，不改`packages/*`、root E2E、Phase 3命令或全局Playwright配置。
- 新指标本机10组初步配对三轮raw/baseline/incremental p95为`320.4/64.7/259.9ms`、`313.3/63.7/259.6ms`和`331.1/65.9/279.2ms`，均低于原`700ms`增量预算；临时4倍CPU节流下则为`1303.6/63.6/1257.3ms`，增量本身明确超限，证明配对基线不会把真实滚动回归变成假绿。所有CDP节流临时代码已从正式差异删除。30组正式focused为`318.3/65.1/267.1ms`并通过；完整`pnpm test:e2e:phase3`为Chromium/Firefox `136 passed / 4 skipped`、WebKit `67 passed / 3 skipped`、perf-chromium `4 passed`，其中Gate 3大文档/Alpha增量p95为`19.8/0ms`，Gate 4 composite raw/baseline/incremental p95为`277.1/66.6/221.6ms`。Phase 3 architecture `7/7`、`pnpm typecheck`、`pnpm lint`和两类diff check均exit `0`。该本地证据仅关闭本轮实现门禁，仍须在新clean SHA的共享runner上通过six-handoff pipeline。
- clean SHA `e62d14751ff65eb03e3fed2e88bf53f85770bc60`触发run `30085965418`；`source-gates`通过，`artifact-build`越过新Gate 4 composite指标并到达visual，随后由`gate2-remediation-justify-table-baseline-visual-chromium-linux.png`与`gate4-narrow-viewport-baseline-visual-chromium-linux.png`两个未跟踪输出触发repository clean-check fail-closed。仓库当时只有Darwin snapshot，Linux CI必然生成缺失平台基线；该run未上传run-a，下游consumer/reproducibility/audit/final均跳过，两个最终ID继续pending/not-generated。
- 以当前`packages/*`和运行态为准复核全部visual seam后，Gate 4 media/table控件确认分别位于`insert`/`table`真实toolbar tab，旧测试直接点击非活动tabpanel内隐藏按钮；Gate 3旧页尾坐标点击也不再等价于当前selection hit-test。最小实现仅让Gate 4 helper复用既有`activateToolbarTab()`，让Gate 3经fixture公开`selectTextRange()`调用当前`Editor.setSelection()`并恢复`editor.focus()`；不修改生产代码、Playwright全局配置、Phase 3命令、阈值、JWL1或后续Phase。
- 正式差异新增五张Playwright `1.59.1` Noble Linux真实snapshot，并按当前固定viewport重生成五张Darwin snapshot；没有复制或改名跨平台图片。五张Linux图片尺寸分别为`1100x900`、`1366x900`、`1280x820`、`1280x900`和`366x756`，像素/可视检查确认均非空且对应Gate 2分页表格、Gate 4 desktop、上传失败弹窗、长表格和窄视口。隔离Linux容器生成后不带`--update-snapshots`复跑8/8，本机正式`pnpm test:visual`也为8/8。
- 扩大验证为Gate 3 focused visual 1/1、Phase 3 architecture 7/7、`pnpm typecheck`、`pnpm lint`、`pnpm test:types`、`git diff --check`和`git diff --cached --check`全部exit `0`。combined implementation review为Standards、Spec各`PASS / 0 finding`；本段简单状态回写按用户决定不重复双轴确认。临时visual配置已删除，隔离副本已移入系统废纸篓；用户原有`.playwright-cli/`与`node-compile-cache/`保持未处理。该证据只解除下一次clean SHA提交前门禁，six-handoff全绿前继续保持B5/JWL1 hard stop。
- attempt 1/2均未上传有效run-a，下游`artifact-consumers`、`artifact-reproducibility`、`artifact-audit`和`artifact-final`按依赖跳过，未生成可写入authoritative ledger的`artifactSetId`或`finalVerificationSha256`。只有新clean SHA上的six-handoff全绿并完成本地final verifier复核后，才能进入B5。
- visual修复后的clean head `60531440293ec3db37e0a7987fd8b7cb301afd40`触发run `30088688871`；`source-gates`、唯一run-a的`artifact-build`和独立run-b compare的`artifact-reproducibility`依次通过，证明此前source、direct gates、E2E、visual、benchmark及reproducibility blocker均已关闭。`artifact-consumers`在`node-exports-types--npm`首个Node probe失败，`artifact-audit`与`artifact-final`按依赖跳过；本run仍没有可关单的`artifactSetId`或`finalVerificationSha256`。
- 真实run-a中的`@4xian/jword-license` tarball `package.json`按当前`packages/license`声明包含`@noble/curves: 2.2.0`，但loopback registry生成的npm metadata只含`name/version/type/dist`，npm因此未解析该运行时依赖，最终由`dist/index.js`抛`ERR_MODULE_NOT_FOUND`。最小synthetic public seam先稳定复现相同缺依赖错误；metadata改为从hash绑定的真实tarball读取`package/package.json`并保留`dependencies`/`peerDependencies`后，npm转绿，同时暴露第二个License物理副本离开pnpm隔离依赖层级的问题。副本改为放在已安装License包的同级解析层后，两个runtime仍保持不同入口与handle fail-closed，且都能解析传递依赖；未修改`packages/license`、依赖版本、JWL1、artifact schema或Phase 3以外调用方。
- 上一版本地反馈环虽为`phase3-third-party-consumers.test.ts` 6/6、3个受影响architecture文件26/26及静态门禁全绿，但combined review发现synthetic根manifest同时直接声明base/leaf，删除metadata修复后不会重现运行时缺依赖，且consumer runner为1021行。新回归先以根manifest仍含base+leaf稳定转红；最小实现让根manifest只直接请求leaf，base仅经leaf tarball metadata解析，并按npm hoist/pnpm隔离的实际依赖解析层读取完整realpath闭包。首轮remediation review为Standards `PASS / 0 finding`、Spec `2 findings`：第4.3/5/6节仍残留根直接列完整closure的旧规则，resolver和handoff重读也未证明realpath位于对应临时project。三处冻结规则已统一为closure只能经tarball metadata解析；新增外部realpath回归先以1/7稳定转红，运行时改为按物理project/repo根验证containment，handoff重读要求全部路径共享同一`<installId>/node_modules/`项目根结构。首次实现因macOS `/var`到`/private/var`的系统symlink暴露词法/物理路径差异，改为先解析project/repo根realpath后，同一consumer test为7/7，3个受影响architecture文件为27/27；`pnpm lint`、`pnpm typecheck`、`pnpm test:types`及两类diff check均exit `0`。最新exact diff combined review为Standards、Spec各`PASS / 0 finding`；consumer runner为999行，其余修改code/test为642/464/936行。tracked差异只含consumer runner、共享artifact utility、consumer project helper、本architecture test与本ledger，用户原有`.playwright-cli/`和`node-compile-cache/`保持未处理。当前已达`Implementation Ready / final evidence pending`，仍需新clean SHA远端consumer、audit、final全绿；通过前不得进入B5或JWL1。
- consumer remediation clean SHA `6220b7e8327e066292920c950ea3128eea57a1b6`对应run `30093740318`继续通过source、run-a和reproducibility，但browser consumer只输出无tuple的`page.waitForFunction` 30秒超时。仅增加失败上下文的clean SHA `14de1ec4edee01b33a0bcc76b798bbdb67a1c459`对应run `30095501904`保持三个前置job通过，并把断点精确收敛为`react-wrapper/npm/vite-browser/firefox`；两个run的audit/final都按依赖跳过，仍没有可关单的最终ID。
...
#### P3-B4 artifact audit assembly与size诊断（2026-07-24）

- install-set修复后的clean SHA `ec99f6b5cf457010a1a3a7af2994e0a00484bddf`触发run `30102749949`；`source-gates`、`artifact-build`、`artifact-consumers`和`artifact-reproducibility`通过，`artifact-audit`在约4分11秒后以`assembly install failed`失败，`artifact-final`按依赖跳过。相同run-a/consumer evidence在`linux/x64`、Node `20.19.0`、npm `10.8.2`、pnpm `9.14.2`下约4分12秒复现，受限stderr为本地scoped registry的`ERR_SOCKET_TIMEOUT`/`ERR_PNPM_META_FETCH_FAIL`；端口可连接但registry callback为0，外部registry为HTTP 200，排除404、外网/TLS、版本和peer冲突。
- 最小throwaway对照证明同进程HTTP server配合同步`spawnSync()`稳定失败，只把等待改为异步后通过；新增public assembly seam回归在旧实现约750ms稳定红，正式实现仅把assembly install改为异步`execFile()`，audit/list仍保留原始stdout与同步证据路径。同一回归转绿，真实CLI在约15.5秒越过customer install，不再发生4分钟socket timeout。
- 真实customer audit随后返回有效JSON、`4 low + 5 moderate + 0 high + 0 critical`且exit 1；按本阶段high/critical门槛新增反例并修正分类后越过。下一断点是pnpm list列出lockfile中`optional: true`、OS/CPU为Android arm64但Linux x64未物化的`@napi-rs/canvas-android-arm64`路径；生成端只记录实际物化节点，final verifier不再对已销毁的上一runner绝对路径无条件`realpathSync()`，但继续要求record来自raw list、contract直接依赖齐全、路径绝对规范且位于仓库外。
- combined review随后指出两项中等级blocker：final verifier按evidence自身过滤raw list会放行被删除的已物化传递依赖；audit exit 1路径会让moderate advisory掩盖payload/工具错误。两个public seam均先稳定转红；修复后生成端只允许父包真实optional声明进入独立记录，final verifier要求raw闭包二选一并用assembly lockfile复核`optional: true`，audit同时拒绝顶层error字段和任意stderr。release gates、assembly seam、artifact build合计`22/22`通过；`pnpm lint`、`pnpm typecheck`、`pnpm test:types`及两类diff check均exit `0`。
- 现存真实run-a tarball重跑同一public assembly seam后，customer生成84项物化依赖和10项未物化optional记录，server生成17项物化依赖且无未物化optional；final verifier随即从两份raw list、assembly evidence和lockfile独立重读，两套均通过。该反馈环直接消费真实package contract和12个tarball，不以旧测试fixture替代packages事实。
- 上述修复的首轮exact diff combined review为Standards `PASS / 0 finding`、Spec `1 finding`：若把raw中的已物化节点从`dependencies[]`同步移入`unmaterializedOptionalDependencies[]`，并提供子snapshot `optional: true`，旧verifier仍会按evidence自报分类放行。新增public verifier反例先稳定以13项中1项失败转红；最小实现继续不扩充handoff schema，而是在当前runner拒绝实际存在的optional路径，并在跨job路径已销毁时同时要求父snapshot的`optionalDependencies`精确指向子name/version及子snapshot为`optional: true`。第二轮exact diff combined review仍为Standards `PASS / 0 finding`、Spec `1 finding`：同名block读取未限定`snapshots:` section，可由`packages:`内伪造父子optional字段绕过；对应反例先稳定以assembly 3项中1项失败转红，最小实现只截取顶层`snapshots:`内容再匹配父子block。第三轮exact diff combined review继续为Standards `PASS / 0 finding`、Spec `1 finding`：同一未物化子节点被多个父包引用时，内部Map会只保留最后一条父边；对应多父反例先稳定以assembly 3项中1项失败转红，最小实现按“子节点+父name/version”保留并逐项验证全部边，公开evidence schema不变。真实pnpm lockfile的`packages`/`snapshots`重复key也已进入回归；customer真实84/10与server真实17/0 evidence重读继续通过，focused release/assembly/artifact三文件现为`23/23`，等待新exact diff combined review。
- 真实CLI已完成customer/server两套安装、audit和dependency list，随后唯一执行blocker收敛为size：Core `612816 / 650000`与native registry `1093 / 1093`通过，六个Vanilla tuple均为`859055 / 700000`。用户于2026-07-25明确选择方案2；六项Vanilla budget只在机器contract和公开architecture期望中统一改为`900000`，完整保留入口JS、CSS与`modulepreload`统计且不修改packages。公开seam先以旧contract六项仍为`700000`稳定红，再以新contract六项`900000`转绿；重新下载run `30102749949`的原始run-a/consumer handoff后，同一production size函数重算8项全部`passed`，六个Vanilla均为`859055 / 900000`且source hash一致。expanded验证为4个相关architecture文件`26/26`、`pnpm lint`、`pnpm typecheck`、`pnpm test:types`与两类diff check全部通过。
- `900000` rebaseline首轮冻结snapshot ID为`66ce52705ab5309bb4fad0159058d336ef8093fea7e7ba5be5c918657c4548cc`；combined reviewer回报Standards `PASS / 0 finding`、Spec `1 finding`，指出`docs/current-implementation/release-artifact-contract.md`仍声明六项Vanilla为`700000`。公开contract test先增加机器/人类契约一致性断言并以缺少`900000`、完整入口统计和禁止自动抬高三段文本稳定转红；人类契约只修改Size段，记录真实基线`859055`、预算`900000`、余量`40945`/约`4.77%`及入口JS、CSS、Vite `modulepreload`统计后，同一test转为`3/3`。修复后4个相关architecture文件为`26/26`，`pnpm lint`、`pnpm typecheck`、`pnpm test:types`及两类diff check均exit `0`；等待包含人类契约的最终exact diff combined review。
- 包含人类契约的九文件snapshot ID `567e0d6e9757ad2fb2f2bd5946191246c8858abef655830abda182e0b44222ab`复审为Standards `PASS / 0 finding`、Spec `1 finding`：上一runner路径销毁后，可把原已物化optional节点移入未物化集合，并以合法optional父子lockfile边通过final verifier。既有public verifier seam新增唯一跨job反例：先物化平台兼容的Linux/x64 optional路径，再删除目录并同步重分类；旧实现稳定接受。最小实现不扩handoff schema，只把run-a冻结`os/arch`传入verifier，并要求每个未物化optional同时由assembly lockfile `packages:`的`os`/`cpu`约束排除该环境；路径仍存在时继续优先报`optional dependency path is materialized`。同一反例转绿，assembly公开文件为`3/3`；仓库真实lockfile探针确认Android/arm64在Linux/x64下排除、Linux/x64 GNU不排除。4个相关architecture文件、`pnpm lint`、`pnpm typecheck`、`pnpm test:types`及两类diff check全部exit `0`，等待新exact diff combined review。
- 新snapshot ID `261f3311f8fae7fdac802638139279d599a7c897ad07b53640f34c5afa5d1704`复审继续为Standards `PASS / 0 finding`、Spec `1 finding`：若同步把assembly lockfile的兼容`os`/`cpu`篡改为Android/arm64，同一可变handoff仍可自证。公开seam在跨job路径已删除后同步修改raw分类、evidence与assembly lockfile，旧实现稳定接受；最小修复继续不扩schema，保留assembly lockfile对optional父子边和平台约束的校验，并追加由run-a `lockfileSha256`及当前checkout身份先行绑定的repo `pnpm-lock.yaml`作为独立平台约束真源。同步篡改反例转绿，原合法未物化、缺父边、多父边、路径仍物化和跨job重分类反例均保持通过；完整assembly文件为`3/3`，等待最终exact diff combined review。
- 随后的九文件exact diff复审为Standards `PASS / 0 finding`、Spec `1 finding`：可把已物化Linux/x64 raw节点同步改名为可信repo lockfile中的Android/arm64 optional，同时保留明确指向原Linux package的旧`.pnpm`路径，旧verifier仍接受。公开verifier seam加入该唯一身份替换反例并在实现修改前稳定转红；最小修复先要求omitted路径绑定pnpm package name/version，再从可信repo lockfile枚举每个父snapshot的完整optional子集合，拒绝删除或改名兼容节点。修复后合法fixture也必须显式保留Android、Linux和缺失optional的完整集合；同一定向反例转绿，四个相关architecture文件为`26/26`，`pnpm lint`、`pnpm typecheck`、`pnpm test:types`及两类diff check全部exit `0`，等待新九文件exact diff combined review。
- snapshot `cb39f7241716668cc09edc269a62c1d6c50189110c293d54b2812d66dc43fcbf`复审为Standards `PASS / 0 finding`、Spec `1 finding`：子optional身份替换已关闭，但把已物化的transitive父节点改名并保留原父包`.pnpm`路径时，可信lock缺少伪造父snapshot会被当作空optional集合，旧verifier仍返回`PARENT_IDENTITY_SUBSTITUTION_ACCEPTED`。同一public verifier seam先加入父身份替换反例并稳定转红；第一步最小实现把pnpm name/version路径校验统一扩到所有非根级raw dependency，既有多父fixture同时改为真实virtual-store路径，反例转绿且真实仓库全部transitive path identity探针通过。冻结下一快照前的只读协调篡改探针又证明，若同步把已销毁的跨job父路径字符串也改成伪造身份，verifier仍返回`PARENT_IDENTITY_AND_PATH_SUBSTITUTION_ACCEPTED`；第二步反例再次稳定转红，最终实现要求非根级父snapshot存在于可信repo lockfile，并让每条omitted parent edge同时由assembly/repo两份lockfile证明。原探针转为`REJECTED`，真实仓库optional完整集合仍通过，四个相关architecture文件保持`26/26`，等待新的九文件exact diff combined review。
- 九文件snapshot `e791baf55fb8a1d43bad4a0ee41aa373828595ec5020b0f764a71308192fc93c`复审为Standards `PASS / 0 finding`、Spec `1 finding`：根级无可信snapshot例外未限定为contract required first-party，可把改名后的transitive父节点提升为额外direct root、保留其他package的旧`.pnpm`路径并同步raw/evidence，从而删除可信optional集合，公开探针返回`EXTRA_DIRECT_PARENT_SUBSTITUTION_ACCEPTED`。同一公开verifier seam新增额外direct反例后先以1项稳定转红；最小实现只要求raw direct的name/version集合与contract requiredDependencies精确一致，保留required first-party根级例外和既有missing-required错误语义。定向两项回归转绿，四个相关architecture文件恢复`26/26`，`pnpm lint`、`pnpm typecheck`、`pnpm test:types`及两类diff check全部exit `0`，等待新九文件exact diff combined review。
- 下一冻结snapshot `dd1aa8ae6d01156031b3188bb0d8f9beb06ee23e175f7fd2d7dd3b0cb82edc4f`复审期间，主进程继续按计划第15项核验“仅直接根级first-party可无repo snapshot”，确认requiredDependencies还包含external/externalPeers，旧实现会让required external同时跳过`.pnpm`身份和可信snapshot，公开探针返回`REQUIRED_EXTERNAL_WRONG_PATH_ACCEPTED`。同一公开seam分两步增加external错误路径与正确路径但缺snapshot的反例并分别稳定转红；最小实现从required集合派生`@4xian/*` first-party direct keys，只允许该集合跳过根级`.pnpm`身份和可信snapshot，external direct必须满足两项门禁。expanded旧fixture把`typescript`路径写成系统临时目录且没有repo snapshot，已按真实pnpm形态改为`.pnpm/typescript@5.9.3/node_modules/typescript`并提供合法内联空snapshot；snapshot parser只追加对精确`{}`空block的识别。定向两项和四个相关architecture文件均恢复通过，最终仍为`26/26`，`pnpm lint`、`pnpm typecheck`、`pnpm test:types`及两类diff check全部exit `0`，等待新的九文件exact diff combined review。
- 随后的exact diff combined review为Standards `PASS / 0 finding`、Spec `1 finding`：上述中间实现仍把repo workspace lock误作整个assembly闭包的精确版本与optional集合allowlist，真实run `30102749949`则合法解析到repo lock不同版本的`@swc/helpers@0.5.23`、`ws@8.21.1`等依赖，且`vue@3.5.39`存在合法peer-context差异。职责收窄后的首轮复审又回报Standards `1 finding`、Spec `2 findings`：assembly lock snapshot parser在helper与final verifier重复且内联`{}`处理分叉；raw `core -> assembly-only`普通父子边未绑定assembly lock；本节验收摘要仍保留旧`23/23`、`84/10`和等待size rebaseline表述。当前最小修复已把snapshot解析、optional标记和optional父子边证明统一收敛到assembly helper，并让每条raw普通/optional父子边都必须由assembly snapshot的精确name/version声明；缺失边public seam先稳定转红，补齐精确`dependencies`边后转绿。repo lock继续只复核实际未物化optional的父子精确边和平台排除，不参与完整图或版本选择。公开seam保留external错误`.pnpm`身份拒绝、额外direct拒绝、raw二选一、跨job重分类、assembly-only transitive及peer-context合法反例；四个相关architecture文件为`26/26`，`pnpm lint`、`pnpm typecheck`、`pnpm test:types`均exit `0`。保留的真实customer/server目录经当前`readResolvedAssemblyDependencies()`和final verifier重读为`85/9`与`17/0`并通过，其中customer目录同时实际物化Linux x64 GNU与musl包，因此分类以当前物理路径为准，不沿用旧`84/10`计数；等待当前exact diff combined review。
- 九文件snapshot `46e82c1e244312e7ea1a5be58de894450cb860d245b2dd7397b7b17f29cccb14`复审为Standards `PASS / 0 finding`、Spec `2 findings`：普通父子边只校验raw到lock，未拒绝lock仍声明但raw删除的传递子节点；被assembly lock `os`/`cpu`排除的optional节点在跨job路径消失后仍可伪装进`dependencies[]`。同一public verifier seam分别增加lock额外已物化子节点和平台排除节点伪装物化两个唯一反例并稳定转红；最小实现按name/version/物理path聚合同一节点的循环或重复raw展开，让每个匹配peer-context snapshot的普通加optional完整子集合与raw双向精确相等，并拒绝任何被assembly lock平台约束排除的`dependencies[]`记录。两个反例转绿，真实customer/server图继续为`85/9`与`17/0`并通过；四个相关architecture文件保持`26/26`，`pnpm lint`、`pnpm typecheck`、`pnpm test:types`均exit `0`，等待新的九文件exact diff combined review。
- 九文件snapshot `98d6a97d7438c56c2991dfb7423b9e67690b26eaaec1c9ccd64f392964ed2939`复审为Standards `PASS / 0 finding`、Spec `1 finding`：同一name/version存在多个peer-context snapshot时，旧verifier只按依赖集合选择任一candidate，没有把raw `.pnpm`物理路径绑定到具体snapshot，因此peer-A路径可搭配peer-B依赖图通过。现有public verifier seam先增加唯一错配反例并稳定转红；最小实现按pnpm v9 virtual-store目录编码把每个raw物理路径精确映射到单一snapshot，再校验该snapshot的普通加optional完整子集合，无法从根级路径消除多snapshot歧义时fail closed。同一反例转绿，真实customer/server图继续按当前物理目录重读为`85/9`与`17/0`并通过；四个相关architecture文件为`26/26`，`pnpm lint`、`pnpm typecheck`、`pnpm test:types`均exit `0`，等待新的九文件exact diff combined review。
- 九文件snapshot `c744dd9f1c676690847118534d0852172fd928ced3a115d47dd033b9a8ade509`复审为Standards `PASS / 0 finding`、Spec `2 findings`：同一raw路径可命中两条相同identity/filename但子图不同的snapshot并由候选择一；未物化peer-A路径还可借用peer-B snapshot的`optional: true`。两个public verifier反例分别先让同一assembly文件稳定为`1 failed / 2 passed`；最小实现要求每个路径恰好命中一个snapshot，并让optional标记校验接收raw物理路径、只读取该唯一peer-context，同时保留错误`.pnpm`身份优先报错的既有语义。两轮同一文件均恢复`3/3`，四个相关architecture文件为`26/26`，真实customer/server evidence经当前helper与final verifier重读继续为`85/9`与`17/0`并通过；`pnpm lint`、`pnpm typecheck`、`pnpm test:types`及两类diff check均exit `0`，等待新的九文件exact diff combined review。
- 九文件snapshot `aac69c3851e1154d45924b294d188c789cbab690822441ffa13e58d5a82b550d`复审为Standards `PASS / 0 finding`、Spec `2 findings`：同一child以相同name/version同时列入父snapshot的`dependencies`和`optionalDependencies`时会被Map合并接受；父edge指向peer-A resolution但raw child路径绑定peer-B时，解析器丢失peer suffix后仍会接受。两个反例依次通过同一公开`validateAssemblyDependencyEvidence()` seam建立红灯，均稳定为assembly文件`1 failed / 2 passed`；第一步最小实现拒绝普通与optional集合重叠，第二步保留父edge完整resolution并复用pnpm v9短目录及长路径MD5/base32编码，将其与raw child的virtual-store目录逐边绑定。每轮同一文件均恢复`3/3`，长peer路径正例保持通过；扩大验证为四个相关architecture文件`26/26`，真实customer/server evidence由当前helper与final verifier重读为`85/9`与`17/0`并通过，`pnpm lint`、`pnpm typecheck`、`pnpm test:types`及四类whitespace检查均通过。当前等待包含本轮修复的新九文件exact diff combined review。
- 九文件snapshot `a9509993818ccecb3fb8a1b6f06c123a81f199ca21b1e2d8b21412806c5be9a5`复审为Standards `PASS / 0 finding`、Spec `2 findings`：同一物理父节点经两个raw根重复展开且同名/version child分别绑定peer-A与peer-B路径时，内部Map会以后一次edge覆盖前一条；长package base identity按pnpm v9截断加hash后不再以完整identity开头，旧预检会把合法路径误报为invalid。两个公开seam反例依次让同一assembly文件稳定为`1 failed / 2 passed`；第一步只允许重复父节点的完全相同child path/optional分类合并，冲突立即fail closed，第二步让路径预检复用`pnpmSnapshotFilename()`接受长base identity的精确哈希目录，既有短identity peer前缀逻辑不变。每轮同一文件均恢复`3/3`；四个相关architecture文件为`26/26`，真实customer/server evidence继续由当前helper与final verifier重读为`85/9`与`17/0`并通过，`pnpm lint`、`pnpm typecheck`、`pnpm test:types`均通过。当前等待新的九文件exact diff combined review。
- 九文件snapshot `c94790c175782aea393397280d7f610802e1e0723c94c08e95290599baf81fed`复审为Standards `PASS / 0 finding`、Spec `1 finding`：长package base identity叠加peer-context时，完整snapshot规范化后会生成与base-only不同的MD5/base32哈希，旧预检既不等于base-only哈希也不以完整base identity开头，因此错误拒绝合法路径。组合正例先让assembly文件稳定为`1 failed / 2 passed`；最小实现只在base identity超过93字符时接受精确120字符的pnpm hash目录形状，随后仍由assembly lock完整snapshot identity重算hash并唯一绑定，错误hash或peer-context继续在图校验阶段fail closed。独立字面hash按pnpm先规范peer分隔符再MD5/base32的规则生成，同一文件恢复`3/3`；四个相关architecture文件为`26/26`，真实customer/server evidence保持`85/9`与`17/0`并通过，`pnpm lint`、`pnpm typecheck`、`pnpm test:types`均通过。当前等待新的九文件exact diff combined review。
- 九文件snapshot `c64780d403942f417261346fe03818c25750581c87df0120b2e4e328be746715`复审为Standards `PASS / 0 finding`、Spec `2 findings`：独立重复展开的同一物理父节点若分别提供不同child子集合，旧实现会把两次展开合并成并集；`.pnpm/<entry>/forged/node_modules/<name>`一类virtual-store entry与`node_modules`不相邻的路径也会通过身份预检。两个唯一反例依次让同一assembly文件稳定为`1 failed / 2 passed`，最小实现先要求独立重复父节点的完整child map完全一致，再要求`.pnpm/<entry>/node_modules`三段连续。扩大到保留的真实customer证据时，`vue -> @vue/server-renderer -> vue`循环回边按pnpm list真实形态使用空child map终止递归，原反馈环稳定报`customer assembly dependency graph is invalid`；新增同形public正例先转红，最终只允许当前递归栈内同一物理父节点的空回边，不放宽独立重复展开或非空循环回边。focused文件恢复`3/3`，四个相关architecture文件为`26/26`，真实customer/server evidence重读为`85/9`与`17/0`并通过，`pnpm lint`、`pnpm typecheck`、`pnpm test:types`及四类whitespace检查均通过。当前等待新的九文件exact diff combined review。
